...
onMounted(async () => {
  try {
    ...
    idError.value = 'No se pudo abrir el vault de identidad (id.closer.click).'
    console.warn(e)
  }
  startLocate()
...
window.addEventListener('appinstalled', () => { canInstall.value = false; deferredPrompt.value = null })
async function install () {
  if (!deferredPrompt.value) return
  deferredPrompt.value.prompt()
  await deferredPrompt.value.userChoice
  canInstall.value = false; deferredPrompt.value = null
}
</script>
